...
public struct OrbView: View {
    ...
    public var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                if viewModel.config.showBackground {
                    background
                }
            ...
            .modifier(
                RealisticShadowModifier(
                    colors: viewModel.config.showShadow ? viewModel.config.backgroundColors : [.clear],
                    radius: size * 0.08
                )
            )
        }
    ...
    private var background: some View {
        LinearGradient(colors: viewModel.config.backgroundColors,
                       startPoint: .bottom,
                       endPoint: .top)
    }
    ...
    private var particleView: some View {
        ...
        ZStack {
            ParticlesView(
                color: viewModel.config.particleColor,
                speedRange: 10...20,
                sizeRange: 0.5...1,
                particleCount: 10,
                opacityRange: 0...0.3
    ...
    private var wavyBlob: some View {
        GeometryReader { geometry in
            ...
            RotatingGlowView(color: .white.opacity(0.75),
                             rotationSpeed: viewModel.config.speed * 1.5,
                           direction: .clockwise)
                .mask {
                    WavyBlobView(color: .white, loopDuration: 60 / viewModel.config.speed * 1.75)
    ...
    private var wavyBlobTwo: some View {
        GeometryReader { geometry in
            ...
            RotatingGlowView(color: .white,
                             rotationSpeed: viewModel.config.speed * 0.75,
                           direction: .counterClockwise)
                .mask {
                    WavyBlobView(color: .white, loopDuration: 60 / viewModel.config.speed * 2.25)
    ...
    private func coreGlowEffects(size: CGFloat) -> some View {
        ZStack {
            RotatingGlowView(color: viewModel.config.glowColor,
                          rotationSpeed: viewModel.config.speed * 3,
                          direction: .clockwise)
                .blur(radius: size * 0.08)
                .opacity(viewModel.config.coreGlowIntensity)

            RotatingGlowView(color: viewModel.config.glowColor,
                          rotationSpeed: viewModel.config.speed * 2.3,
                          direction: .clockwise)
    ...
    private func baseDepthGlows(size: CGFloat) -> some View {
        ZStack {
            ...
            RotatingGlowView(color: viewModel.config.glowColor.opacity(0.5),
                          rotationSpeed: viewModel.config.speed * 0.25,
                          direction: .clockwise)
